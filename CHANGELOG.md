...
- **PDF export without LibreOffice keeps the pad's font (#8245).** The built-in PDF renderer, which is what runs on every install that has no `soffice` configured, only ever drew with pdfkit's Helvetica and Courier, so a `font-family` applied in the pad (`ep_font_family`, or anything else styling text) was dropped — while HTML, ODT and DOCX carried it correctly. Font families in the export HTML are now mapped onto the PDF standard fonts by category, so serif, sans-serif and monospace text stay distinct, with bold and italic variants honoured. Operators who need an exact face can register font files through the new `exportPdfFonts` setting; a missing or unreadable file degrades to a built-in font instead of failing the export, and pads with no font styling produce byte-identical output to before.
