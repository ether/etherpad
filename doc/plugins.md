...
than hidden. Nothing is hidden on a failed lookup — if the npm registry cannot
be reached the full catalog is listed — and installing such a plugin by hand
with `pnpm run plugins i ep_<name>` is still possible.
