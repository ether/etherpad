...
// Selectable themes GÇö orthogonal to light/dark mode. `value` is the
// data-theme attribute slug used in pad.css [data-theme="GÇª"] selectors;
...
// Light/dark mode is orthogonal GÇö paired with the theme in CSS via
...
const buildThemeRow = (selectId) => {
  ...
  // which only fires jQuery-bound handlers GÇö not native addEventListener. Bind
  // through jQuery so the widget actually drives applyMarginTheme.
  $(select).on('change', () => {
    applyMarginTheme(select.value);
    // Mirror to the sibling select (user-settings Gåö pad-settings).
...
const injectThemeSelector = () => {
  ...
  sections.forEach(({sectionId, dropdownId, checkboxId}) => {
    ...
    if (!document.getElementById(checkboxId)) {
      ...
      // order GÇö matching the natural top-to-bottom flow of the section.
...
window.customStart = () => {
  ...
  // Inject the Theme selector into Settings GåÆ user-settings section.
  ...
  } catch (_) { /* quota / private mode GÇö skip silently */ }
};
