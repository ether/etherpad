...
const mapAuthorWithDBKey = async (mapperkey: string, mapper:string) => {
  ...
  if (author == null) {
    ...
    const author = await createAuthor(null as unknown as string);
 ...
 */
export const setAuthorColorId = async (author: string, colorId: string|null|undefined) => {
 ...
 * @param {String} name The name of the author
 */
export const setAuthorName = async (author: string, name: string|null|undefined) => {
 ...
 */
export const anonymizeAuthor = async (
    authorID: string,
    opts: {dryRun?: boolean} = {},
): Promise<{
  affectedPads: number,
