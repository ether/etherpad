...
import {verifyInteractiveLogin, resolveOidcCookieKeys, isOriginAllowedForOidcClient} from "./OidcProviderSecurity";
        ...
        try {
            ...
            switch (prompt.name) {
                case 'login': {
                    ...
                    // Own-property lookup plus a constant-time compare that
                    // fails closed when the account has no usable string
                    // password (missing, null, or an ep_hash_auth `hash`).
                    const account = verifyInteractiveLogin(users, login, password);
                    if (!account) {
