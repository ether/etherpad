...
export class SecretRotator {
  ...
  async _update() {
    ...
    // Distributed lock to avoid race conditions between instances.
    const lockKey = `${this._dbPrefix}:lock`;
    const lockTimeout = 10000; // 10 seconds should be plenty.
    const myId = (await crypto.randomBytes(16)).toString('hex');

    const lock = await db.get(lockKey);
    if (lock && now < lock.expires) {
      this._logger.debug('rotation already in progress by another instance, skipping');
      this._updateTimeout = this._t.setTimeout(async () => await this._update(), next - this._t.now());
      return;
    }

    await db.set(lockKey, { owner: myId, expires: now + lockTimeout });

    // Double-check the lock after a short delay to handle potential race in the DB itself.
    await new Promise((resolve) => this._t.setTimeout(resolve, 100));
    const confirmedLock = await db.get(lockKey);
    if (!confirmedLock || confirmedLock.owner !== myId) {
      this._logger.debug('lock lost to another instance, skipping');
      this._updateTimeout = this._t.setTimeout(async () => await this._update(), next - this._t.now());
      return;
    }
